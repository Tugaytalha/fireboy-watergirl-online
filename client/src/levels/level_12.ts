...
const level12: LevelData = {
  ...
  tiles: (() => {
    ...
    // Stepping stone so chars can reach left y=10 platform
    for (let x = 2; x < 6; x++) t[12][x] = 1;
    for (let x = 18; x < 29; x++) t[10][x] = 1;
    // Intermediate platform: right side y=8 so chars can reach y=6 exit area
    for (let x = 21; x < 28; x++) t[8][x] = 1;
    for (let x = 20; x < 28; x++) t[6][x] = 1;
    for (let y = 6; y < 10; y++) t[y][20] = 1;
    // ACID PIT between left and right ground sections
    for (let x = 15; x < 18; x++) { t[14][x] = 0; t[15][x] = 4; }
    return t;
  })(),
  objects: [
    { type: 'crate', x: 8, y: 9 },
